...
# diacritic2composite - converts use of combining diacritics (like
#   e&comacute;) to a composite character (like &eacute;) when possible
...
# used to build a hash of entity names.
...
my ($test, $buffer, $match, $comp, $out);

$me = 'diacritic2composite';
$usage = <<EOD;

$me - converts use of combining diacritics (like
  e&comacute;) to a composite character (like &eacute;) when possible

Usage: $me filenames

In:  XML file(s) that use mnemonic/named character entity references
...
while (<DATA>) {
    if (/^(.+?),(.+?),/) {
        $entityName = $1;
        $charCode = $2;
	$charents{$entityName} = '' unless exists($charents{$entityName});
    }
}
...
foreach $infile (@ARGV) {
	...
	if ( /(\w)&com(acute|breve|caron|cedil|circ|dot|grave|macr|ogon|ring|tilde|uml);/ ) {
	    $test = $_;
	    $buffer = '';
	    while ( $test =~ /(\w)&com(acute|breve|caron|cedil|circ|dot|grave|macr|ogon|ring|tilde|uml);/ ) {
		$match = $&;
		$comp = $1 . $2;
		$buffer .= $`;
		$test = $';
		if ( exists($charents{$comp}) ) {
		    $out = '&' . $comp . ';';
		} else {
		    $out = $match;
		}
