	...
	if (scalar(@ARGV) eq 3) {
		...
		if (open(INPUT,"$inputDirectory\/$inputFile")) {
			...
			if (open(OUTPUT,"> $outputDirectory/$outputFile")) {
				print(OUTPUT "Surrogate No.\tPID\tDigital File Name\tProject Name\n");
				...
				if (($fileNameCol ne -1) and ($projectNameCol ne -1) and ($surrogateNumCol ne -1)) {
					...
					while ($line = <INPUT>) {
						...
						$pid =~ s/\.[a-z]{3,4}$//i;
						print(OUTPUT "@columns[$surrogateNumCol]\t$pid\t$digitalFileName\t@columns[$projectNameCol]\n");
					}
				} else {
