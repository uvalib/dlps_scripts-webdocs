...
my $version = "1.2";
my $versionDate = "2005-07-29";

# 2004-09-24: 1.0: gpm2a: First production version
#
...
# be.
#
# 2005-07-27: gpm2a: Added -l (label) option for specifying a disc
# label template other than the default. Added list of directories and
# disc sequence number to merge-file output for optional inclusion on
# the disc label.
...
# Project overseen by Jama Coartney <jsc3x@virginia.edu>.
...
my ($me, $usage, $usageSummary, %opts, $copies, $media, $format, $label);
...
my (%filelist, $editlistFile, $mergeFile, $logFile, $errorLogFile, %parentDirs);
...
my $IMAGE_DIR = 'K:/Images/';
my $DEFAULT_LABEL = $RIMAGE_ROOT . 'Labels/DEFAULT.btw';
my $TIBET_LABEL = $RIMAGE_ROOT . 'Labels/tibet.btw';
...
$me = 'rimage.pl';
$usageSummary = "Usage: $me [-b base-id] [-c copies] [-k] [-l dlps|tibet]
  [-m DVD|CD] [-t] [-v] [-I|-P] paths";
$usage = <<EOD;
...
$usageSummary
  ...
  -l (label)      Informal name of label template: use 'dlps' or 'tibet'
                  (defaults to dlps)
...
if ($label) {
    ...
    } elsif ($label eq 'tibet') {
	$LABEL_FILE = $TIBET_LABEL;
    } else {
	die "If used, -l option must be either 'dlps' or 'tibet'\n$usageSummary\n";
    }
} else {
    $LABEL_FILE = $DEFAULT_LABEL;
}
...
# redirect standard error to .err error log in Rimage logs directory
unless ( $opts{t} ) {
    if ( chdir($RIMAGE_ROOT) ) {
        $CONFIG_FILE =~ s:/:\\:g;  # convert to backslashes
        #my $retval = system("java edu.virginia.lib.dlps.rimage.CLUI $CONFIG_FILE 2> $errorLogFile");
        my $retval = system("java edu.virginia.lib.dlps.rimage.CLUI $CONFIG_FILE");
...
sub addFile {
	...
	if ($label eq 'tibet') {
	    ...
	    # get name of parent directory - the directory containing the file
	    if ( $filepath =~ m:([^/]+?)/[^/]+$: ) {
		$parentDir = $1;
		# add parent directory to hash (for inclusion in merge file)
		$parentDirs{$parentDir} = '';
...
sub writeConfigFile {
    ...
    # details needed by the DLPSRimage Java application
...
<DLPSRimageConfig copies="$copies" media="$media" isVerbose="$verbose" imageOnly="$imageOnly" productionOnly="$productionOnly" deleteImages="$deleteImages">
    <options>
        ...
        <dtds>
            <imageOrderDTD>C:\\Rimage\\XML\\ImageOrder_1.1.dtd</imageOrderDTD>
            <productionOrderDTD>C:\\Rimage\\XML\\ProductionOrder_1.3.DTD</productionOrderDTD>
        </dtds>
